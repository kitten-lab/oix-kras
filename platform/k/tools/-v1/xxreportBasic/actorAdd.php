...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // CHANGE PER TOOL //
  $POST__REPORT_TOPIC =   $_POST['POST__REPORT_TOPIC']; 
  $POST__REPORT_TEXT =    $_POST['POST__REPORT_TEXT']; 
  $POST__REPORT_TYPE =    $GLOBALS['roomkey'];


    ## TOOL SIG FILE
    $TOOL_FUNC = "MADE A REPORT";
    $TOOL_LOC = "reportBasic";
    $TOOL_NAME = "actorSubmitReport";
        ## SET YOUR KDE FOR THIS TOOL ##
        $KDE__ERROR_TYPE = $TOOL_FUNC . " DUPLICATE REJECTED";
        $KDE__SOURCE = $TOOL_NAME;
        $KDE__ECHO_CHAIN = $TO__SYS . ' > ' . $TO__DOM . ' > ' . $TO__MOD;
        $KDE__ERROR = "";
        ################################
...
$ROUTE = $GLOBALS['sonar'] . $SHADOW_PROD_TOGGLE . $ROUTE__LINE . $TOOL_LOC . '/' . $TO__SYS . '/' . $TO__DOM . '/';
    if (!is_dir($ROUTE)) { mkdir($ROUTE, 0775, true); }   
    
  $CHEST = $ROUTE . '/' . $POST__REPORT_TYPE . '_data.json';
  ...
  $CHEST_THINGS[$cUID] = [
    ...
    "TO__MOD" => $TO__MOD,
    "POST__MOD" => $POST__MOD, 
    "POST__REPORT_TOPIC" => $POST__REPORT_TOPIC, 
    "POST__REPORT_TEXT" => $POST__REPORT_TEXT,
    "POST__REPORT_TYPE" => $POST__REPORT_TYPE,
